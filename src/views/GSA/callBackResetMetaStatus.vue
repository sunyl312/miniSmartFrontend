  ...
  <v-sheet min-height="600">
    ...
    <expand-panel>
      ...
      <v-sheet class=" px-3 py-6" outlined rounded>
        ...
        这个命令的应用场景是：审编人员审核不通过后，用户根据指示又修改了信息。则需要此命令将审核状态从已审核重置为未审核，以此来再次审核。<br/>
    ...
    <v-row no-gutters>
      <v-col cols="3">
        <v-sheet class="my-1 ml-2 mr-1 pa-3" min-height="600" outlined rounded>
          ...
          <v-sheet>
            <v-btn :disabled="dataIDListStr ==null" style="color: white" :loading="isLoading" @click="loadData" :color="$store.state.globalColor" class="mx-2 my-1"  small>
          ...
          <v-sheet>
            <v-textarea :disabled="isLoading" v-model="dataIDListStr" background-color="yellow lighten-4" clearable flat height="550"
                        hide-details
                        label="subCRA ids"
                        outlined></v-textarea>
          </v-sheet>
